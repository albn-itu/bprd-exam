...
void printInstruction(word p[], word pc) {
  ...
  case CALL:
    ...
    break;
  case TCALL:
    printf("TCALL " WORD_FMT " " WORD_FMT " " WORD_FMT, p[pc + 1], p[pc + 2],
           p[pc + 3]);
    break;
  case RET:
...
void heapStatistics() {
  word blocks = 0, free = 0, orphans = 0, blocksSize = 0, freeSize = 0,
       largestFree = 0;
  word *heapPtr = heap;
  ...
  while (freePtr != 0) {
    ...
    if (freePtr < heap || afterHeap < freePtr + length + 1) {
      printf("HEAP ERROR: freelist item " WORD_FMT " (at heap[" WORD_FMT
  ...
  printf("Heap: " WORD_FMT " blocks (" WORD_FMT " words); of which " WORD_FMT
         " free (" WORD_FMT " words, largest " WORD_FMT " words); " WORD_FMT
         " orphans\n",
...
#if defined(ENV64)
  if (sizeof(word) != 8 || sizeof(word *) != 8 || sizeof(uword) != 8) {
    printf("Size of word, word* is not 64 bit, cannot run\n");
    return -1;
  }
#elif defined(ENV32)
  if (sizeof(word) != 4 || sizeof(word *) != 4 || sizeof(uword) != 4) {
